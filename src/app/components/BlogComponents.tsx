...
export function FeaturedCard({ post }: FeaturedCardProps) {
  return (
    <Link href={`/blog/${post.slug}`} className="group block">
      <article className="relative bg-gradient-to-r from-blue-600 to-purple-600 rounded-2xl overflow-hidden text-white">
        {post.heroImage && (
...
export function CategoryBadge({ category, variant = 'default' }: CategoryBadgeProps) {
  const colors = {
    'Guides': variant === 'light' ? 'bg-green-100 text-green-800' : 'bg-green-600 text-white',
    'News': variant === 'light' ? 'bg-red-100 text-red-800' : 'bg-red-600 text-white',
    'Tools': variant === 'light' ? 'bg-blue-100 text-blue-800' : 'bg-blue-600 text-white',
    'Analysis': variant === 'light' ? 'bg-purple-100 text-purple-800' : 'bg-purple-600 text-white',
    'Getting Started': variant === 'light' ? 'bg-yellow-100 text-yellow-800' : 'bg-yellow-600 text-white',
    'Tutorial': variant === 'light' ? 'bg-indigo-100 text-indigo-800' : 'bg-indigo-600 text-white',
  }
